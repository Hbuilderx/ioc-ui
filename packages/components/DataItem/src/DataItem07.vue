...
<template>
    <div class="box">
        ...
        <div class="total" ref="total">
          <span v-for="n in cubeCount" :style="{background:'#0F3D61',width:cubeWidth+'px'}"></span>
        </div>
    ...
    methods:{
        initStatus(cubeCount){
          ...
          for(let i=0;i<this.endColor.length;i++){
            perDiffColor.push( Number(((this.endColor[i]-this.startColor[i])/nowCubeCount).toFixed(3)))
          }
